...
(function ($, window, document, undefined) {
  $.fn.jConveyorTicker = function (options) {
    ...
    this.each(function () {
      ...
      $list.removeAttr('style').children('li').removeAttr('style');
      $wrap.addClass('jctkr-wrapper');

      var conveyorInit = function conveyorInit() {
        ...
        var conveyorAnimate = function conveyorAnimate(action) {
          var tw = $list.width();
        ...
        if (reverse_commute) {
          $wrap.prev('.jctkr-label').hover(function () {
            $list.stop();
            conveyorAnimate('reverse');
          }, function () {
            $list.stop();
      ...
      if ($listRawWidth >= $parent1stThreshold) {
        ...
        var itemsReplicate = function itemsReplicate() {
          var $listItems1stClone = $list.clone().children('li');
          $list.append($listItems1stClone);
