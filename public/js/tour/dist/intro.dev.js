...
(function (root, factory) {
  ...
  function _introForElement(targetElm) {
    ...
    if (this._options.steps) {
      ...
      for (var i = 0, stepsLength = this._options.steps.length; i < stepsLength; i++) {
        var currentItem = _cloneObject(this._options.steps[i]); //set the step
      ...
      var allIntroSteps = targetElm.querySelectorAll('*[data-intro]'); //if there's no element to intro
      ...
      for (var i = 0, elmsLength = allIntroSteps.length; i < elmsLength; i++) {
        var currentElement = allIntroSteps[i]; // skip hidden elements
    ...
    self._introItems = introItems; //add overlay layer to the page
  ...
  function _placeTooltip(targetElement, tooltipLayer, arrowLayer, helperNumberLayer, hintMode) {
    ...
    switch (currentTooltipPosition) {
      ...
      case 'bottom-left-aligned': // Bottom-left-aligned is the same as the default bottom
  ...
  function _determineAutoPosition(targetElement, tooltipLayer, desiredTooltipPosition) {
    ...
    var targetOffset = _getOffset(targetElement); // If we check all the possible areas, and there are no valid places for the tooltip, the element
    // must take up most of the screen real estate. Show the tooltip floating in the middle of the screen.


    var calculatedPosition = "floating"; // Check if the width of the tooltip + the starting point would spill off the right side of the screen
  ...
  function _setHelperLayerPosition(helperLayer) {
    if (helperLayer) {
      ...
      var currentElement = this._introItems[this._currentStep],
          ...
          widthHeightPadding = 10; // If the target element is fixed, the tooltip should be fixed as well.
  ...
  function _showElement(targetElement) {
    ...
    var self = this,
        ...
        elementPosition = _getOffset(targetElement.element); //check for a current step highlight class
    ...
    if (oldHelperLayer != null) {
      ...
      self._lastShowElementTimer = setTimeout(function () {
        ...
        oldReferenceLayer.querySelector('.introjs-progress .introjs-progressbar').setAttribute('style', 'width:' + _getProgress.call(self) + '%;'); //show the tooltip
      ...
      referenceLayer.className = 'introjs-tooltipReferenceLayer'; //set new position to helper layer
  ...
  function _scrollTo(scrollTo, targetElement, tooltipLayer) {
    ...
    if (!_elementInViewport(targetElement.element)) {
      ...
      var bottom = rect.bottom - winHeight; // TODO (afshinm): do we need scroll padding now?
  ...
  function _populateHints(targetElm) {
    ...
    if (this._options.hints) {
      ...
      for (var i = 0, l = hints.length; i < l; i++) {
        var currentElement = hints[i]; // hint animation
    ...
    if (document.addEventListener) {
      document.addEventListener('click', _removeHintTooltip.bind(this), false); //for window resize
  ...
  function _addHints() {
    ...
    for (var i = 0, l = this._introItems.length; i < l; i++) {
      var item = this._introItems[i]; // avoid append a hint twice
  ...
  function _getOffset(element) {
    ...
    if (element instanceof SVGElement) {
      ...
      elementPosition.height = element.offsetHeight; //calculate element top and left
  ...
  introJs.version = VERSION; //Prototype
