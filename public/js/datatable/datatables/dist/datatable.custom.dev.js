...
$(document).ready(function () {
  $('product-list').DataTable(); // Basic table example 
  ...
  var eventFired = function eventFired(type) {
    var n = $('#demo_info')[0];
    n.innerHTML += '<div class="me-2"><b>' + type + ' event - </b>' + new Date().getTime() + '</div>';
    n.scrollTop = n.scrollHeight;
  };

  $('#advance-2').on('order.dt', function () {
    eventFired('Order');
  ...
  $('#example-style-8').DataTable(); // Data sources tables
  ...
  $('#addRow').click();
  $('#addRow').click();
  $('#addRow').click(); // Setup - add a text input to each footer cell
  ...
  $('#datatable-ordering').DataTable(); //Ordering Plug-ins (with type detection) end here
