...
(function ($) {
  ...
  $("#batchDelete").jsGrid({
    ...
    fields: [{
      headerTemplate: function headerTemplate() {
        return $("<button>").attr("type", "button").text("Delete").addClass("btn btn-danger btn-sm btn-delete mb-0").click(function () {
          deleteSelectedItems();
        });
      },
      itemTemplate: function itemTemplate(_, item) {
        return $("<input>").attr("type", "checkbox").prop("checked", $.inArray(item, selectedItems) > -1).on("change", function () {
  ...
  var deleteClientsFromDb = function deleteClientsFromDb(deletingClients) {
