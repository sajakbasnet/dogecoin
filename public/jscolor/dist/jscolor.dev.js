...
if (!window.jscolor) {
  window.jscolor = function () {
    ...
    var jsc = {
      ...
      installBySelector: function installBySelector(selector) {
        var elms = document.querySelectorAll(selector); // for backward compatibility with DEPRECATED installation/configuration using className
      ...
      fetchElement: function fetchElement(elementOrSelector) {
        ...
        if (_typeof(elementOrSelector) === 'object' && elementOrSelector instanceof HTMLElement) {
          // HTML element
          return elementOrSelector;
        }

        console.warn('Invalid element of type %s: %s', _typeof(elementOrSelector), elementOrSelector);
        return null;
      },
      ...
      pub: function pub(targetElement, opts) {
        ...
        presetsArr.push('default'); // let's apply the presets in reverse order, so that should there be any overlapping options,
        ...
        this.fromHSVA = function (h, s, v, a, flags) {
          ...
          if (a !== null) {
            ...
            this.channels.a = this.isAlphaEnabled() ? Math.max(0, Math.min(1, this.maxA, a), this.minA) : 1.0; // if alpha channel is disabled, the color should stay 100% opaque
          }
        ...
        this.toBackground = function () {
          ...
          backgrounds.push(jsc.genColorPreviewGradient(this.toRGBAString())); // data URL of generated PNG image with a gray transparency chessboard
        ...
        this.isAlphaEnabled = function () {
          ...
          if (this.alphaChannel === 'auto') {
            return this.format.toLowerCase() === 'any' || // when the format is 'any', it can change on the fly (e.g. from hex to rgba), so let's consider the alpha channel enabled
        ...
        function drawPicker() {
          ...
          if (!jsc.picker) {
            jsc.picker = {
              ...
              asld: document.createElement('div'),
              // alpha slider
              asldB: document.createElement('div'),
              // border
              asldM: document.createElement('div'),
              // mouse/touch area
              asldGrad: jsc.createASliderGradient(),
              asldPtrS: document.createElement('div'),
              // slider pointer spacer
              asldPtrIB: document.createElement('div'),
              // slider pointer inner border
              asldPtrMB: document.createElement('div'),
              // slider pointer middle border
              asldPtrOB: document.createElement('div'),
              // slider pointer outer border
          ...
          p.box.style.height = dims[1] + 'px'; // picker shadow
          ...
          p.asldM.style.cursor = 'default'; // alpha slider pointer inner and outer border
        ...
        function onValueChange(ev) {
          ...
          jsc.triggerCallback(THIS, 'onChange'); // triggering valueElement's onChange
    ...
    jsc.pub.install = function () {
      jsc.installBySelector('[data-jscolor]'); // for backward compatibility with DEPRECATED installation using class name

      if (jsc.pub.lookupClass) {
        jsc.installBySelector('input.' + jsc.pub.lookupClass + ', ' + 'button.' + jsc.pub.lookupClass);
      }
